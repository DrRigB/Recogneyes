...
public class FaceDetector : MonoBehaviour
{
    ...
    [Header("Recognition Integration (No-DB placeholders)")]
    [Tooltip("If true and no database/manager is available, use placeholder names like 'Person <ID>'.")]
    public bool UsePlaceholderRecognition = true;
    [Tooltip("Optional external recognizer hook: provide a function that takes a grayscale face ROI Mat and returns (name, confidence). If set, this is used instead of RecognitionManager.")]
    public Func<Mat, (string name, double confidence)> ExternalRecognizer = null; // not shown in Inspector
    public event Action<int> OnFaceConfirmed; // (faceId)
    public event Action<int, string, double> OnFaceRecognized; // (faceId, name, confidence)

    [Header("Text To Speech")]
    public TTSService TTS;  // Assign in Inspector
    public bool EnableTTS = true;
    public bool SpeakOnFaceConfirm = true;
    public bool SpeakOnRecognition = true;
    [Range(0f, 10f)]
    public float TTSCooldownSeconds = 3f;
    ...
    // TTS state
    private bool[] _hasSpokenOnConfirm = new bool[MaxFaceBoxes];
    private string[] _lastSpokenName = new string[MaxFaceBoxes];
    private float[] _lastSpokenAt = new float[MaxFaceBoxes];
            ...
            if (mainCam != null && _faceBoxRenderers != null)
            {
                ...
                for (int i = 0; i < faces.Length && i < MaxFaceBoxes; i++)
                {
                    ...
                    if (_consecutiveDetections[i] >= StableDetectionFrames)
                    {
                        ...
                        if (_consecutiveDetections[i] == StableDetectionFrames)
                        {
                            ...
                            OnFaceConfirmed?.Invoke(_faceIDs[i]);
                            // TTS: Speak on first confirmation per face slot
                            if (EnableTTS && SpeakOnFaceConfirm && TTS != null)
                            {
                                if (Time.time - _lastSpokenAt[i] >= TTSCooldownSeconds && !_hasSpokenOnConfirm[i])
                                {
                                    string confirmText = ShowFaceIDs ? $"Face detected. ID {_faceIDs[i]}" : "Face detected";
                                    TTS.Speak(confirmText);
                                    _lastSpokenAt[i] = Time.time;
                                    _hasSpokenOnConfirm[i] = true;
                                }
                            }
                            
                            // FACE RECOGNITION: Identify who this person is
                            if (ShowRecognizedNames)
                            {
                    ...
                    else if (_isConfirmedFace[i] && _frameCount % 30 == 0)
                    {
                        if (ShowRecognizedNames)
                        {
                            PerformRecognition(i, face);
                    ...
                    // TTS: Speak when recognition result appears or changes
                    if (EnableTTS && SpeakOnRecognition && TTS != null && ShowRecognizedNames)
                    {
                        string currentName = _recognizedNames[i];
                        if (!string.IsNullOrEmpty(currentName) && currentName != "Unknown")
                        {
                            bool nameChanged = _lastSpokenName[i] != currentName;
                            bool cooldownPassed = (Time.time - _lastSpokenAt[i]) >= TTSCooldownSeconds;
                            if ((nameChanged || string.IsNullOrEmpty(_lastSpokenName[i])) && cooldownPassed)
                            {
                                TTS.Speak(currentName);
                                _lastSpokenName[i] = currentName;
                                _lastSpokenAt[i] = Time.time;
                            }
                        }
                    }
                ...
                for (int i = 0; i < MaxFaceBoxes; i++)
                {
                    ...
                    else if (_framesSinceLastSeen[i] > FacePersistenceFrames)
                    {
                        ...
                        _hasSpokenOnConfirm[i] = false;
                        _lastSpokenName[i] = null;
    ...
    private void PerformRecognition(int faceIndex, OpenCvSharp.Rect faceRect)
    {
        try
        {
            ...
            // Priority 1: External recognizer hook (plug your DB-backed recognizer here)
            if (ExternalRecognizer != null)
            {
                try
                {
                    var result = ExternalRecognizer.Invoke(faceROI);
                    name = result.name;
                    confidence = result.confidence;
                }
                catch (Exception hookEx)
                {
                    Debug.LogWarning($"External recognizer failed: {hookEx.Message}");
                }
            }
            // Priority 2: Built-in RecognitionManager if present and ready
            else if (RecognitionManager != null && RecognitionManager.IsReady())
            {
                var result = RecognitionManager.RecognizeFace(faceROI);
                name = result.Item1;
                confidence = result.Item2;
            }
            // Fallback: Placeholder recognition (no DB) to keep UX flowing
            else if (UsePlaceholderRecognition)
            {
                name = $"Person {_faceIDs[faceIndex]}";
                confidence = 0.0; // Placeholder, not meaningful
            }
            else
            {
                name = "Unknown";
            ...
            if (name != "Unknown")
            {
                ...
                OnFaceRecognized?.Invoke(_faceIDs[faceIndex], name, confidence);
            }
            else
            {
