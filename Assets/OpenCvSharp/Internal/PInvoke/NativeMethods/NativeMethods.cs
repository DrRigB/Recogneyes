...
#pragma warning restore CA1031
    ...
    /// <summary>
    /// Implementation of error handler that throws exceptions
    /// </summary>
    [AOT.MonoPInvokeCallback(typeof(CvErrorCallback))]
    private static int ErrorHandlerThrowExceptionImpl(ErrorCode status, string funcName, string errMsg, string fileName, int line, IntPtr userData)
    {
        throw new OpenCVException(status, funcName, errMsg, fileName, line);
    }

    /// <summary>
    /// Implementation of error handler that ignores errors
    /// </summary>
    [AOT.MonoPInvokeCallback(typeof(CvErrorCallback))]
    private static int ErrorHandlerIgnoranceImpl(ErrorCode status, string funcName, string errMsg, string fileName, int line, IntPtr userData)
    {
        return 0;
    }
